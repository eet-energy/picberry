...
void dspic33ckxxmp10x::send_cmd(uint32_t cmd)
{
	uint8_t i;
...
inline void dspic33ckxxmp10x::send_prog_nop(void)
{
	uint8_t i;
...
uint16_t dspic33ckxxmp10x::read_data(void)
{
	uint8_t i;
	uint16_t data = 0;
...
/* enter program mode */
void dspic33ckxxmp10x::enter_program_mode(void)
{
	int i;
...
/* exit program mode */
void dspic33ckxxmp10x::exit_program_mode(void)
{
	GPIO_CLR(pic_clk);
	GPIO_CLR(pic_data);
	delay_us(DELAY_P16);
...
bool dspic33ckxxmp10x::read_device_id(void)
{
	bool found = 0;
...
uint8_t dspic33ckxxmp10x::blank_check(void)
{
	uint32_t addr;
	unsigned short i;
...
/* Bulk erase the chip */
void dspic33ckxxmp10x::bulk_erase(void)
{
	if(flags.debug) cerr << "Erasing memory";

	if(flags.debug) cerr << "Erasing memory of dspic33CK";
	...
	/**** ATTENTION!  ****/
	/**** INCORRECT DATASHEET -  SEQUENCE FOUND BY REVERSE ENGINEERING ****/
	// Unlock erase/write operation
	send_cmd(0x200558);
	send_cmd(0x8846B8);
	send_cmd(0x200AA9);
	send_cmd(0x8846B9);
...
void dspic33ckxxmp10x::read(char *outfile, uint32_t start, uint32_t count)
{
...
void dspic33ckxxmp10x::write(char *infile)
{
	uint16_t i;
	uint16_t k;
	bool skip;
	...
	const char num_config_regs = 14;
	const char *regname[] = {"FSEC","FBSLIM","FOSCSEL","FOSC","FWDT", "FPOR", "FICD", "FDMTIVTL", "FDMTIVTH", "FDMTCNTL", "FDMTCNTH", "FDMT", "FDEVOPT", "FALTREG"};
	...
	for (addr = 0; addr < mem.code_memory_size; ){
		...
		send_cmd(0x884693);	//MOV W3, NVMADR
		send_cmd(0x8846A4);	//MOV W4, NVMADRU

		/* Set the NVMCON to program two instruction words */
		send_cmd(0x24001A); // MOV #0x4001, W10
		send_nop();
		send_cmd(0x88468A); // MOV W10, NVMCON
		send_nop();
		send_nop();
	...
	for(i=0; i<num_config_regs; i++)
	{
		...
		if(mem.filled[addr]){
			...
			send_cmd(0x200001 | ((((uint32_t)mem.location[addr] | 0xFF0000) & 0x00FF0000) >> 12) );
			send_cmd(0x200002 | ((0x0000FFFF & mem.location[addr+1]) << 4));
			send_cmd(0x200003 | ((((uint32_t)mem.location[addr+1] | 0xFF0000) & 0x00FF0000) >> 12) );
			...
			send_cmd(0x884694);	//MOV W4, NVMADR
			send_cmd(0x8846A5);	//MOV W5, NVMADRU
	...
	if(!flags.noverify){
		...
		for(unsigned short i=0; i<num_config_regs; i++)
		{
			send_nop();
			send_nop();
			send_nop();
			...
			config_data = (0x00FFFF & ((hbyte << 16) | lbyte));
...
void dspic33ckxxmp10x::dump_configuration_registers(void)
{
